pipeline {
  ...
  environment {
    ...
    EXT_REPO = 'pairdrop'
    BUILD_VERSION_ARG = 'PAIRDROP_VERSION'
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-pairdrop'
    CONTAINER_NAME = 'pairdrop'
    DOCKERHUB_IMAGE = 'linuxserver/pairdrop'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/pairdrop'
    PR_DOCKERHUB_IMAGE = 'lspipepr/pairdrop'
    DIST_IMAGE = 'alpine'
    MULTIARCH='true'
    CI='true'
    CI_WEB='true'
    ...
    stage('Build-Single') {
      ...
      steps {
        ...
        sh "docker buildx build \
          ...
          --label \"org.opencontainers.image.description=[PairDrop](https://github.com/schlagmichdoch/PairDrop) is a sublime alternative to AirDrop that works on all platforms. Send images, documents or text via peer to peer connection to devices in the same local network/Wi-Fi or to paired devices.\" \
